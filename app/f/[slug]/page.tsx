...
export type FunnelData = {
  ...
  // Card 1 – Hero
  heroHeadline: string
  heroSubheadline: string   // sales pitch line
  heroCopy: string          // body copy paragraph
  heroBgImage: string
  heroFontSize?: number     // px override; omit to use auto-fit

  // Card 2 – Video
  videoUrl: string
  videoProductName: string  // kept for peek-layer label during bounce

  // Card 3 – Offer
  offerTitle: string
  offerPrice: string        // optional price display, e.g. '$49'
...
export const defaultFunnelData: FunnelData = {
  ...
  heroSubheadline: 'One sentence sales pitch about your product or service.',
  heroCopy:
    'Add your supporting copy here. Tell visitors exactly what makes this different and why they need it right now.',
  heroBgImage: '',

  videoUrl: '',
  videoProductName: 'Product Name',

  offerTitle: 'Your Offer',
  offerPrice: '',
  offerBenefits: [
    'First key benefit of your product',
    'Second key benefit of your product',
    'Third key benefit of your product',
  ],
  ctaText: 'Buy now',
  ctaLink: '#',
  trustLine: 'Free shipping · 30-day returns',
}
...
const STYLES = `
  ...
  /* ── Card-2 peek layer (behind scroll container, shown during bounce) ── */
  .fv-peek {
    position: absolute;
    bottom: 0;
    left: 0;
  ...
  .fv-heading {
    font-family: var(--fv-font-heading), Inter, sans-serif;
  }
  ...
  .fv-chevron-in  {
    animation:
      fv-fade-in  0.4s ease forwards,
      fv-bob 1.6s 0.4s ease-in-out infinite;
  }
...
// ─── useFitText ───────────────────────────────────────────────────────────────
// Binary-searches for the largest font-size (px) at which the element's
// scrollHeight stays within maxHeight. Re-runs whenever `text` changes.
...
function useFitText(
  ...
  useEffect(() => {
    ...
    // Headline may occupy at most 50% of viewport height
    ...
    // Leave the measured size on the element; state catches up on next render
    setSize(lo)
  // ref is stable — intentionally omitted from deps
...
interface Props {
  funnelData?: Partial<FunnelData>
  previewMode?: boolean   // set true in builder preview: skips consent + animations
}

export default function FunnelPage({ funnelData: overrides, previewMode }: Props) {
  const data: FunnelData = { ...defaultFunnelData, ...overrides }
  ...
  // ── Scroll to card 3 when video ends ────────────────────────────────────────
  ...
  useEffect(() => {
    if (previewMode) return   // builder preview: skip consent + animations
    const decided = localStorage.getItem('fv_consent_decided')
    if (decided) {
      // Return visitor: no consent screen, bounce after 2.5 s
  ...
  const handleConsent = (accepted: boolean) => {
    ...
    // Chevron + bounce fire after consent is dismissed
    ...
    <>
      ...
      <div className="fv-outer">
        <div className="fv-inner">
          ...
          {/* ── Card-2 peek layer (behind scroll container) ───────────────── */}
          ...
          <div ref={scrollRef} className={scrollClass}>
            ...
            <div className="fv-card" style={{ background: heroBg }}>
              <div style={{
                ...
                {/* Headline — font size auto-fit, overridable via heroFontSize */}
                <h1
                  ref={headlineRef}
            ...
            <div className="fv-card" style={{ background: '#000' }}>
              ...
              {/* Sound toggle — only meaningful when a video is present */}
              {data.videoUrl && (
                <button
                  onClick={toggleSound}
        ...
        <div className="fv-consent-backdrop">
          <div className="fv-consent-card">
            ...
            <div style={{
              ...
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
                <path
                  d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 15h-1v-5h1v5zm0-7h-1V8h1v2z"
